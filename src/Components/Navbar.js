...
const Navbar = () => {
  ...
  return (
    ...
    <div>
          ...
          <div className='flex lg:mr-0 items-center font-aeonikmedium text-sm'>
            <div className='flex items-center space-x-4'>
              <a href='https://wa.me/+2349071504491?text=Hello,%20Majorlink%20I%20would%20like%20to%20reach%20out%20to%20the%20support%20team.%20'>
                <button className=' text-black bg-[#fff]  lg:block hidden p-1 lg:px-8 px-6 rounded-full lg:h-10 lg:mt-0 lg:mr-0  hover:border-gray-400'>Trade on WhatsApp</button>
              </a>
              <Link to={'/checkout'}>
                <button className=' text-black bg-[#fff]  lg:block hidden p-1 lg:px-8 px-6 rounded-full lg:h-10 lg:mt-0 lg:mr-0  hover:border-gray-400'>Trade now</button>
              </Link>
            </div>

            {/*Menu Icon*/}
            <span className='mt-3 text-3xl lg:hidden cursor-pointer' onClick={() => setOpen(!open)}>
      ...
      {open === false ? <div data-aos="fade-left" className='bg-[#4B5DFF] h-full lg:h-0 text-[#fff] float-right fixed right-0 top-0 z-10  block motion-safe:animate-fadeIn w-[100%] lg:w-0 lg:hidden'>
